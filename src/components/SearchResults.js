...
import SaltCard from "./Salt";
import NoResultsCard from "./NoResultsCard";

const SearchResults = ({ searchText, setLoader, loader }) => {
    ...
    <div className="flex flex-col justify-center items-center bg-gray-100">
      ...
      {saltData?.map((data, index) => (
        <SaltCard
          ...
          productRate="$20"
          saltData={data["salt_forms_json"]}
          name={data["salt"]}
        />
      ))}

      {(!loader && saltData?.length) === 0 ? <NoResultsCard searchText={searchText} /> : <></> }
    </div>
  );
};
